...
def df_problem1(dataframe):
    ...
    Returns the column names starting with a number
    """
    problematic_cols = []
...
def df_problem_solver1(dataframe):
    """
    Takes dataframe as parameter
    Changes the column names starting with a number
    """
    problematic_cols = []
...
def unistats(dataframe,sorted="Missing"):
    ...
    Returns count, missing, unique, dtype, mode..."""
...
def breush_pagan(dataframe,target):
    """
    Takes dataframe and target as parameter
    Applies breush pagan test: tests the variance of error is equal vs not. Large p values indicate high error variance
    Returns the test results as pandas df 
    """
...
def anova(dataframe, target):
    """
    Takes dataframe and target as parameter
    Applies ANOVA test: tests the correlation between categorical columns and target feature
    Returns the test results as pandas df """
...
def histogram(dataframe,feature, target):
    """
    Takes dataframe, feature and target as parameter
    Applies a one way f test between categories for a single categorical column
    Large p value indicates high correlation between categories
    Returns an histogram with the test results """
...
def biv_stats(dataframe, target):
    """
    Takes dataframe and target as parameters
    For numeric columns: ed.scatter
    For categoric colums: ed.histogram and ed.barplots takes place
    """
    for col in dataframe:
        if pd.api.types.is_numeric_dtype(dataframe[col]):
            scatter(dataframe,col,target)
    for col in dataframe:
        histogram(dataframe,col,target)
    for col in dataframe:
        barplots(dataframe,col,target)

def cleaner(dataframe):
    """
    Takes dataframe as parameter
    Drops nan values
    Strips the column names
...
def dummy(dataframe):
    """
    Takes dataframe as parameter
    Creates dummy variables for every categorical column in the dataframe
...
def regression(dataframe,target):
    """
    Takes the dataframe and target as parameter
    Returns the results using all the variables in the dataframe"""   
    y = dataframe[target]
...
def regression_stats_df(results):
    """
    Takes regression results as parameter
    Returns the parameters, t-values and p-values in a dataframe"""
